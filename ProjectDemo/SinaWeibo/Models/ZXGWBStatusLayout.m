...
@implementation ZXGWBStatusLayout
...
- (void)layout {
    ...
    // 计算高度
    _rowHeight = 0;
    _rowHeight += _marginTop;
    _rowHeight += _titleHeight;
    _rowHeight += _profileHeight;
    _rowHeight += _textHeight;
    if (_retweetHeight > 0) {
        _rowHeight += _retweetHeight;
    ...
    if (_tagHeight > 0) {
        _rowHeight += _tagHeight;
    } else {
        if (_picHeight > 0 || _cardHeight > 0) {
            _rowHeight += kWBCellPadding;
        }
    }
    _rowHeight += _toolbarHeight;
    _rowHeight += _marginBottom;
}
...
- (void)layoutTitle {
    ...
    ZXGWBStatusTitle *title = _status.title;
...
- (void)layoutName {
    ...
    // VIP
    if (user.mbrank > 0) {
        UIImage *yelllowVImage = [WBStatusHelper imageNamed:[NSString stringWithFormat:@"common_icon_membership_level%zd",user.mbrank]];
        if (!yelllowVImage) {
...
- (void)_layoutCardWithStatus:(ZXGWBStatus *)status isRetweet:(BOOL)isRetweet {
    if (isRetweet) {
    ...
    ZXGWBPageInfo *pageInfo = status.pageInfo;
    if (!pageInfo) return;
...
- (void)_layoutPicsWithStatus:(ZXGWBStatus *)status isRetweet:(BOOL)isRetweet {
    if (isRetweet) {
    ...
    switch (status.pics.count) {
        case 1: {
            ZXGWBPicture *pic = _status.pics.firstObject;
...
- (NSMutableAttributedString *)_textWithStatus:(ZXGWBStatus *)status
...
#pragma mark - Public
- (NSAttributedString *)_attachmentWithFontSize:(CGFloat)fontSize imageURL:(NSString *)imageURL shrink:(BOOL)shrink { // shrink 缩小
    /*
